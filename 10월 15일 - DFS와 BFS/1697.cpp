...
int minTime(int n, int k) {
	int time = 0;
	vector<bool>visited(MAX + 1, false); //방문 체크할 벡터
	...
	while (!q.empty()) {
		...
		if (cur_position + 1 <= MAX && !visited[cur_position + 1]) { //한 칸 앞으로 이동
		...
		if (cur_position * 2 <= MAX && cur_position * 2 > 0 && !visited[cur_position * 2]) { //2배 앞으로 이동
